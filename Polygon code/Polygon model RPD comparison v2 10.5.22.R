...
LCFsp_function <- function(df, sp){
  ...
  df <- df %>% drop_na()
  df$concat <- paste(df$model,df$Variable, sep=" ", collapse=NULL)
...
region_plot_function<- function(pol, string){
  ...
  BA_model <- sp_function(BAdat, pol)
  ...
  LGMdf <- ldply(list.files(), read.csv, header=TRUE) #files with base at -71 to 4k 
  ...
  mod <- ggplot(LGMdf, aes(x=zt, y = RPD, fill = fill)) + 
    geom_boxplot()+ ggpubr:: theme_pubr() +   
    ggpubr::labs_pubr(base_size = 10) + geom_vline(xintercept = 0, color = 'red')+ 
  ...
  BA_model$fill <- BA_model$meanBA <0
  RPD <- ggplot(BA_model, aes(x=BA, y=model, fill = fill)) + 
...
plot <- ggarrange(map, mod, RPD, LCF,
            labels = c("A", "B", "C", 'D'),
            ncol = 2, nrow = 2, 
            widths = c(1, 1, 1,1),
            legend = NULL)
