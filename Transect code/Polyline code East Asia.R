...
#create lines
#set  line coordinates, transect width and sampling resolution (hcount), enter values for points as lon then lat
l2a <- c(85,23)
l2b<- c(116,23)
res <- 6
lmin2a <- c(l2a[1],l2a[2]-res)
lmin2b <- c(l2b[1],l2b[2]-res)
lmax2a <- c(l2a[1],l2a[2]+res)
lmax2b <-c(l2b[1],l2b[2]+res)
...
LGM_dataseteur <-LGM_dataset %>% filter(lat < lmax2a[2] & lat >lmin2b[2], lon <lmax2b[1] &lon > lmin2a[1])
LGM_dataseteur <-LGM_dataset %>% filter(EST_AGE < 24000 & EST_AGE >17000)



LGM_dataseteur$lat <-as.numeric(LGM_dataseteur$lat)
LGM_dataseteur$longitude <-as.numeric(LGM_dataseteur$longitude)

LGM_dataseteur$ID_ENTITY <-as.factor(LGM_dataseteur$ID_ENTITY)

entboxplot<-ggplot(LGM_dataseteur, aes(x=lon, y= zt, group = ID_ENTITY)) + geom_boxplot(width = 2) +theme_bw()
entboxplot
...
coeff <- 0.1
LGM_dataseteur$ztscaled <- LGM_dataseteur$zt *10
mean <-ggplot(af_mean,aes(x = lat)) + geom_ribbon(aes(ymin= Min, ymax = Max, alpha =0.5))  + geom_boxplot(data = LGM_dataseteur, aes(x=lat, y= zt, group = ID_ENTITY),  width =1, alpha = 0.5) + geom_line(aes(y=Mean, color = 'red')) + theme_bw() + scale_y_continuous(
...
means <- aggregate(LGM_dataseteur$zt, list(LGM_dataseteur$ID_ENTITY), FUN=mean) 
colnames(means) <- c('ID_ENTITY', 'meanzt')
LGM_dataseteur <- merge(LGM_dataseteur, means, by= 'ID_ENTITY')
LGM_dataseteur$fill <- LGM_dataseteur$meanzt <0
LGM_dat <- LGM_dataseteur[c(1,11,12)]
...
SPITFIRE <- ggplot(af_SPITFIRE,aes(x = lon)) + geom_crossbar(aes(y= Mean,ymin=Min, ymax=Max, fill = fill), color = NA, width = 0.5, alpha = 0.5) + geom_line(aes(y=Mean), color = 'purple', size = 1)+ geom_boxplot(data = LGM_dataseteur, aes(x=lon, y= ztscaled, group = ID_ENTITY, fill = fill),  width =0.5, alpha = 0.9) + geom_line(data= subset(LGM_dat, !is.na(meanzt)), aes(x= lon, y = meanzt *10),  color =  'green', linetype = 'dashed', size = 1)+
  scale_y_continuous("BA anomaly", limits= c(-70,70), sec.axis = sec_axis(~.*coeff, name = 'zt')) +scale_x_continuous(limits = c(l2a[1],l2b[1]))+ geom_hline(yintercept=0, linetype="dashed", color = "black", size=0.5) + theme_pubr() +theme(legend.position = "none") + ggtitle("SPITFIRE")+ scale_fill_manual(values=c("FALSE"="red","TRUE"="blue"))
...
ORCHIDEE <-ggplot(af_ORCHIDEE,aes(x = lon)) + geom_crossbar(aes(y= Mean,ymin=Min, ymax=Max, fill = fill),color = NA, width = 0.5, alpha = 0.5) + geom_line(aes(y=Mean), color = 'purple', size = 1)+ geom_boxplot(data = LGM_dataseteur, aes(x=lon, y= ztscaled, group = ID_ENTITY, fill = fill),  width =0.5, alpha = 0.9)  + geom_line(data= subset(LGM_dat, !is.na(meanzt)), aes(x= lon, y = meanzt *10),  color =  'green', linetype = 'dashed', size = 1)+
  scale_y_continuous("BA anomaly", sec.axis = sec_axis(~.*coeff, name = 'zt')) +scale_x_continuous(limits = c(l2a[1],l2b[1])) +geom_hline(yintercept=0, linetype="dashed", color = "black", size=0.5) + theme_pubr() +theme(legend.position = "none") + ggtitle("ORCHIDEE")+ scale_fill_manual(values=c("FALSE"="red","TRUE"="blue"))
...
LPJLM <- ggplot(af_LPJLM,aes(x = lon)) + geom_crossbar(aes(y= Mean,ymin=Min, ymax=Max, fill = fill), color = NA,width = 0.5, alpha = 0.5) + geom_line(aes(y=Mean), color = 'purple', size = 1)+ geom_boxplot(data = LGM_dataseteur, aes(x=lon, y= ztscaled, group = ID_ENTITY, fill = fill),  width =0.5, alpha = 0.9)  + geom_line(data= subset(LGM_dat, !is.na(meanzt)), aes(x= lon, y = meanzt *10),  color =  'green', linetype = 'dashed', size = 1)+ scale_y_continuous(
  "BA anomaly", sec.axis = sec_axis(~.*coeff, name = 'zt'))  +scale_x_continuous(limits = c(l2a[1],l2b[1])) + geom_hline(yintercept=0, linetype="dashed", color = "black", size=0.5) + theme_pubr() +theme(legend.position = "none") + ggtitle("LPJLM")+ scale_fill_manual(values=c("FALSE"="red","TRUE"="blue"))
...
SIMFIRE <- ggplot(af_SIMFIRE,aes(x = lon)) + geom_crossbar(aes(y= Mean,ymin=Min, ymax=Max, fill = fill),color = NA, width = 0.5, alpha = 0.5) + geom_line(aes(y=Mean), color = 'purple', size = 1)+ geom_boxplot(data = LGM_dataseteur, aes(x=lon, y= ztscaled, group = ID_ENTITY, fill = fill),  width =0.5, alpha = 0.9) + geom_line(data= subset(LGM_dat, !is.na(meanzt)), aes(x= lon, y = meanzt *10),  color =  'green', linetype = 'dashed', size = 1) + scale_y_continuous(
  "BA anomaly", sec.axis = sec_axis(~.*coeff, name = 'zt')) +scale_x_continuous(limits = c(l2a[1],l2b[1]))  + geom_hline(yintercept=0, linetype="dashed", color = "black", size=0.5) + theme_pubr() +theme(legend.position = "none") + ggtitle("SIMFIRE")+ scale_fill_manual(values=c("FALSE"="red","TRUE"="blue"))
...
ggplot(data = world) + geom_map(data = world, map = world, aes(long, lat, map_id = region), color = 'dark grey', fill = 'grey') + 
  geom_point(data = LGM_dataseteur, aes(x = lon, y = lat), color = 'red') +
  ...
  scale_x_continuous(limits = c(lmax2a[1]-5,lmax2b[1]+5),breaks = seq(-180, 180, 20)) + 
  scale_y_continuous(limits = c(lmin2b[2]-5, lmax2a[2]+5), breaks = seq(-60, 90, 20))
