...
LGM_dataseteur <-LGM_dataset %>% filter(lat < l2b[2] & lat >l2a[2], lon <l2b[1] &lon > l2a[1])
...
ggplot(data = world) + geom_map(data = world, map = world, aes(long, lat, map_id = region), color = 'dark grey', fill = 'grey') + 
  ...
  scale_x_continuous(limits = c(l2a[1]-30,l2b[1]+30),breaks = seq(-180, 180, 20)) + 
  scale_y_continuous(limits = c(l2a[2]-10, l2b[2]+20), breaks = seq(-60, 90, 20))
