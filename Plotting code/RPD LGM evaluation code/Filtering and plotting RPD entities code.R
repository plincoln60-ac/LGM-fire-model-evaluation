...
tseries <- ggplot(time_series_plot, aes(x= median_age/1000, y=zt)) +  geom_rect(aes(xmin = minval/1000, xmax = maxval/1000, ymin = -Inf, ymax = Inf), fill = "blue",  alpha = 0.2) +
   geom_rect(aes(xmin = basemin/1000, xmax = basemax/1000, ymin = -Inf, ymax = Inf), fill = "red", alpha = 0.2) +
   geom_line(size= 0.5) + scale_x_continuous(limits= c(0,24)) +  ggpubr:: theme_pubr() +ggpubr::labs_pubr(base_size = 6) +
...
Plot_Iteration_A <- Plotting_function_map(LGMdf,Duplicates_Iteration_A,20500,21500,100,4000, 'Iteration A following Power et al. (2008)')
Plot_Iteration_B <- Plotting_function_map(LGMdf,Duplicates_Iteration_B,17480,23290,100,4000, 'Iteration B between H1 & H2')
Plot_Iteration_C <- Plotting_function_map(LGMdf_itc,Duplicates_Iteration_C,17480,23290,100,1000, 'Iteration C finer baseline (0.1-1k)')
...
Plotting_function_errors_map <- function(df,dupdf,minval,maxval, basemin,basemax) {
  ...
  ##################################First filter for duplicates. FILTER HERE INCLUDES UNDERTAINTIES##################################
  ###############################################################################################
  Ldf <-merge(spdf, LGMdf, by =c('ID_ENTITY'))
  Ldf <- Ldf %>% dplyr::filter(median_age + UNCERT_75 <maxval & median_age - UNCERT_25 >minval)%>%
    drop_na(zt) %>%
    subset(!(ID_ENTITY %in% f$ID_ENTITY))  #exclude duplicate entities
  ...
  tseries <- ggplot(time_series_plot, aes(x= median_age/1000, y=zt)) +  geom_rect(aes(xmin = basemin/1000, xmax = basemax/1000, ymin = -Inf, ymax = Inf), fill = "red") +
    geom_rect(aes(xmin = minval/1000, xmax = maxval/1000, ymin = -Inf, ymax = Inf), fill = "blue") +
    geom_line() + scale_x_continuous(limits= c(0,24)) +  ggpubr:: theme_pubr() +ggpubr::labs_pubr(base_size = 6) +
    facet_wrap(as.factor(ID_ENTITY) ~ ., scales = "free_y") 
  ...
  map <- ggplot(data=spdf, aes(x=lon, y=lat)) +
    ...
    geom_point(data = spdf, aes(x = lon, y = lat, color = fill), size =2) +
    geom_text(inherit.aes = T, aes(label =ID_ENTITY), nudge_y = 3, size = 2) +
    geom_text(aes(x= -150, y=-30, label = enttext)) +
    scale_color_manual(values=c("FALSE"="red","TRUE"="blue"), labels = c('Positive', 'Negative'))+
    geom_hline(yintercept = 0, col= 'tan4') +
    geom_hline(yintercept = 30, col='tan1', linetype = 'dashed') +
    geom_hline(yintercept = -30, col='tan1', linetype = 'dashed') +
...
Plot_Iteration_A <- Plotting_function(LGMdf,Duplicates_Iteration_A,20500,21500,100,4000)
Plot_Iteration_A <- Plotting_function_errors(LGMdf,Duplicates_Iteration_A,20500,21500,100,4000)

Plot_Iteration_A
Plot_Iteration_B <- Plotting_function(LGMdf,Duplicates_Iteration_B,17480,23290,100,4000)
Plot_Iteration_B <- Plotting_function_errors(LGMdf,Duplicates_Iteration_B,17480,23290,100,4000)

Plot_Iteration_B
Plot_Iteration_C <- Plotting_function(LGMdf_itc,Duplicates_Iteration_C,17480,23290,100,1000)
Plot_Iteration_C <- Plotting_function_errors(LGMdf_itc,Duplicates_Iteration_C,17480,23290,100,1000)



Plotting_function <- function(df,dupdf,minval,maxval, basemin,basemax) {
