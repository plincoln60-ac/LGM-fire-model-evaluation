...
ORCHIDEELCF <- ORCHIDEELCF %>% pivot_wider(names_from = "vegtype", values_from = "landCoverFrac")
ORCHIDEELGMLCF <- ORCHIDEELGMLCF %>% pivot_wider(names_from = "landcoverfraction", values_from = "pft")
#create reference to left join
#merge dataframes
...
#####Plot data ----
controlBiome <-ggplot(ORCHIDEE1, aes(x=longitude, y=latitude, fill = biomes)) + geom_tile(alpha = 0.5) +  ggtitle('Control Biomes') + geom_point(data = BiomePD, aes(x= lon, y=lat,fill = biomesimple), colour="black",pch=21, size = 0.75)+ geom_map(data = world, map = world, aes(long, lat, map_id = region), size = 0.25, color = 'black', fill = 'transparent') + theme_pubr()+ scale_x_continuous(limits = c(-180,180)) + scale_y_continuous(limits = c(-60,80))+ scale_fill_manual(values = biome6kcol) 
LGMBiome <-ggplot(ORCHIDEELGM1, aes(x=longitude, y=latitude)) + geom_tile(data = ORCHIDEELGM1, aes(fill = biomes), alpha = 0.5) + ggtitle('LGM Biomes') + geom_point(data = BiomeLGM, aes(x= lon, y=lat,fill = biomesimple), colour="black",pch=21)+  geom_map(data = world, map = world, aes(long, lat, map_id = region), size = 0.25, color = 'black', fill = 'transparent') + theme_pubr()+ scale_x_continuous(limits = c(-180,180)) + scale_y_continuous(limits = c(-60,80)) + scale_fill_manual(values = biome6kcol)  + scale_color_manual(values = biome6kcol, guide = FALSE)         

# BA rasters
controlBA <-ggplot(ORCHIDEEBA, aes(x=longitude, y=latitude, fill = burntArea)) + geom_tile(alpha = 0.5) + ggtitle('Control Burnt Area (%)') + geom_map(data = world, map = world, aes(long, lat, map_id = region), size = 0.25, color = 'black', fill = 'transparent') + theme_pubr()+ scale_x_continuous(limits = c(-180,180)) + scale_y_continuous(limits = c(-60,80))+ scale_fill_distiller(palette = "Spectral")
...
LPJLMBA <- tidync::tidync("/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/Biomes/LPJLM/LPJLM_BA_1951_1970.nc")
LPJLMLCF <- tidync::tidync("/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/Biomes/LPJLM/LPJLM_LCF_1951_1970.nc")
LPJLMLGMBA <- tidync::tidync("/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/Biomes/LPJLM/LGM_burntArea.nc")
...
LPJLMBA <- LPJLMBA %>% tidync::hyper_tibble(force = T) # write variable to a dataframe
LPJLMLCF <- LPJLMLCF %>% tidync::hyper_tibble(force = T) # write variable to a dataframe
LPJLMLGMBA <- LPJLMLGMBA %>% tidync::hyper_tibble(force = T) # write variable to a dataframe
LPJLMLGMLCF <- LPJLMLGMLCF %>% tidync::hyper_tibble(force = T) # write variable to a dataframe
head(LPJLMLCF)

LPJLMLCF <- LPJLMLCF %>% pivot_wider(names_from = "pft", values_from = "cover")
LPJLMLGMLCF <- LPJLMLGMLCF %>% pivot_wider(names_from = "pft", values_from = "cover")
#create reference to left join
#merge dataframes
LPJLMLGM <-LPJLMLGMLCF
LPJLM <- LPJLMLCF
...
head(LPJLMLGM1)
#check numbers
table(LPJLMLGM1$biomes)
table(LPJLM1$biomes)


#####Plot data ----

controlBiome <-ggplot(LPJLM1, aes(x=lon, y=lat, fill = biomes)) + geom_tile(alpha = 0.5) +  ggtitle('Control Biomes') + geom_point(data = BiomePD, aes(x= lon, y=lat,fill = biomesimple), colour="black",pch=21, size = 0.75)+ geom_map(data = world, map = world, aes(long, lat, map_id = region), size = 0.25, color = 'black', fill = 'transparent') + theme_pubr()+ scale_x_continuous(limits = c(-180,180)) + scale_y_continuous(limits = c(-60,80))+ scale_fill_manual(values = biome6kcol) 
LGMBiome <-ggplot(LPJLMLGM1, aes(x=lon, y=lat)) + geom_tile(data = LPJLMLGM1, aes(fill = biomes), alpha = 0.5) + ggtitle('LGM Biomes') + geom_point(data = BiomeLGM, aes(x= lon, y=lat,fill = biomesimple), colour="black",pch=21)+  geom_map(data = world, map = world, aes(long, lat, map_id = region), size = 0.25, color = 'black', fill = 'transparent') + theme_pubr()+ scale_x_continuous(limits = c(-180,180)) + scale_y_continuous(limits = c(-60,80)) + scale_fill_manual(values = biome6kcol)  + scale_color_manual(values = biome6kcol, guide = FALSE)         
controlBiome
