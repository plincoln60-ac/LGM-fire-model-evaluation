...
LPJLMBAdiff <- tidync::tidync("/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/Biomes/LPJLM/LPJLM_1951_1970_BA_diff.nc")
LPJLMLGMLCF<-LPJLMLCFfunction(LPJLMLGMLCF)
LPJLMLCF<-LPJLMLCFfunction(LPJLMLCF)
LPJLMBAdiff <- BAfunction(LPJLMBAdiff)
LPJLMBAdiff$BA <- LPJLMBAdiff$BA *100


LPJLMLCF <- LPJLMLCF[,c(1,2,10)]
LPJLMLGMLCF <- LPJLMLGMLCF[,c(1,2,10)]
LPJLM <- LPJLMLCF[,1:2]
LPJLM <- merge(x= LPJLMLCF,y= LPJLMLGMLCF, by = c('lat','lon'), all.x = TRUE)
LPJLM <- merge(x= LPJLM,y= LPJLMBAdiff, by = c('lat','lon'), all.x = TRUE)

colnames(LPJLM) <- c('lat','lon','Reference','LGM','BA_anomaly')
...
Change <-ggplot(data=LPJLM, aes(x=lon, y=lat)) +
...
LGM <-ggplot(data=LPJLM, aes(x=lon, y=lat)) +
...
REF<-ggplot(data=LPJLM, aes(x=lon, y=lat)) +
...
SPITBA <- ggplot(data=LPJLM, aes(x=lon, y=lat)) +
...
annotate_figure(p4, top = text_grob("LPJLM", 
